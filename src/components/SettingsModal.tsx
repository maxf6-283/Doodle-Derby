...
export function SettingsModal(props: {
  ...
  const [eraseKey, setEraseKey] = createSignal(getState("hotkey-erase") ?? "e");
  const [undoKey, setUndoKey] = createSignal(getState("hotkey-undo") ?? "u");
  const [redoKey, setRedoKey] = createSignal(getState("hotkey-redo") ?? "r");
  const [fillKey, setFillKey] = createSignal(getState("hotkey-fill") ?? "f");
  ...
  const handleConfirm = () => {
    ...
    setState("hotkey-erase", eraseKey(), true);
    setState("hotkey-undo", undoKey(), true);
    setState("hotkey-redo", redoKey(), true);
    setState("hotkey-fill", fillKey(), true);
